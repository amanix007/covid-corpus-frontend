...
import CollapsibleTableHead from "./OtherCollapsibleTableHead";
import OtherSubTableRow from "./OtherSubTableRow";
...
export default function OtherCollapsibleTableNEW(props) {
  /* console.log("OtherCollapsibleTable ==> rows", props); */
  const { rows } = props;
  const classes = useStyles();
  const [page, setPage] = React.useState(0);
  const [order, setOrder] = React.useState("asc");
  const [orderBy, setOrderBy] = React.useState("Other_call_name");
            ...
            <Table className={classes.table} aria-label="collapsible table">
              ...
              <TableBody>
                {stableSort(rows, getComparator(order, orderBy))
                  // .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                  .map((row, index) => {
                    return <OtherSubTableRow key={row.id} row={row} />;
                  })}
              </TableBody>
            </Table>
          </TableContainer>
