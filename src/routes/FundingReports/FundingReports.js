...
class FundingReports extends Component {
  state = {
    search: "",
    reportsData: [],
    isLoading: true,
  ...
  searchField(text) {
    ...
    advancedSearchService.searchByParams('?' + params).then(async response => {
      if (response) {
        await this.setState({
          reportsData: response.funding_opportunities,
  ...
  generateParams({ countries, searchTerm, disciplines, startDate, deadlineDate, types, status, open_to_collaborators }) {
    let params = '';
        ...
        <>
          ...
          <Grid container spacing={3} justify="center">
            ...
            <Grid item xs={12} style={{ padding: 0 }}>
              ...
              {!this.state.isLoading && (
                <FundingCollapsibleTable rows={filteredData} />
              )}
            </Grid>
          </Grid>
        </>
